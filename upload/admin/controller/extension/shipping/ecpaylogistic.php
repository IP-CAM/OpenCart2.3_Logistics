...
class ControllerExtensionShippingecpayLogistic extends Controller 
{
	...
	public function index() 
	{
		...
		if (isset($this->error['HILIFE_Collection_fee'])) {
			$data['error_HILIFE_Collection_fee'] = $this->error['HILIFE_Collection_fee'];
		} else {
			$data['error_HILIFE_Collection_fee'] = '';
		}
		...
		if (isset($this->error['HILIFE_fee'])) {
			$data['error_HILIFE_fee'] = $this->error['HILIFE_fee'];
		} else {
			$data['error_HILIFE_fee'] = '';
		}
		...
		$data['action'] = $this->url->link('extension/shipping/ecpaylogistic', 'token=' . $this->session->data['token'], 'SSL');
	...
	{
		if (!$this->user->hasPermission('modify', 'extension/shipping/ecpaylogistic')) {
	...
	public function create_shipping_order() 
	{
		...
		if (!$ecpaylogistic_query->num_rows) {
			...
			if ($order_info) {
				include_once("controller/extension/shipping/ECPay.Logistics.Integration.php");
				include_once("controller/extension/shipping/ecpaylogistic.inc.php");
				include_once("model/extension/shipping/EcpayCartLogisticsLibrary.php");
				$sFieldName = 'code';
				...
				try {
					...
					$AL->Send = array(
						...
						'ServerReplyURL' => (($_SERVER['HTTPS']) ? "https" : "http") . '://' . $_SERVER['HTTP_HOST'] . str_replace("admin/","",$_SERVER['PHP_SELF']) . '?route=extension/shipping/ecpaylogistic/response',
						'LogisticsC2CReplyURL' => (($_SERVER['HTTPS']) ? "https" : "http") . '://' . $_SERVER['HTTP_HOST'] . str_replace("admin/","",$_SERVER['PHP_SELF']) . '?route=extension/shipping/ecpaylogistic/LogisticsC2CReply',
						'Remark' => 'OC2_ECPayLogistic_1.0.0724',
					);
					$AL->SendExtend = array(
	...
	public function show_cvs_map_by_order()
	{
		...
		if ($aorder_info) {
			include_once("controller/extension/shipping/ECPay.Logistics.Integration.php");
			include_once("controller/extension/shipping/ecpaylogistic.inc.php");
			$sServerReply = $this->url->link('extension/shipping/ecpaylogistic/set_store_info_by_order', 'token=' . $this->session->data['token'], true);
	...
	public function install() 
	{
		...
		if (method_exists($this->user,"getGroupId")) {
			...
			$this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/shipping/ecpaylogistic');
			$this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/shipping/ecpaylogistic');
		} 
		include_once("controller/extension/shipping/ecpaylogistic.inc.php");
		$sFieldName = 'code';
		if (!ecpay_column_exists($this->db, DB_PREFIX."setting", 'code')) {
	...
	public function uninstall() 
	{
		...
		if (method_exists($this->user,"getGroupId")) {
			...
			$this->model_user_user_group->removePermission($this->user->getGroupId(), 'access', 'extension/shipping/ecpaylogistic');
			$this->model_user_user_group->removePermission($this->user->getGroupId(), 'modify', 'extension/shipping/ecpaylogistic');
		} 
	}
}
