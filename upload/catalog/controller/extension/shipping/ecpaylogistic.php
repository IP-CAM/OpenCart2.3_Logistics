...
class ControllerExtensionShippingecpayLogistic extends Controller {
	...
	public function show_cvs_map() {
		include_once("admin/controller/extension/shipping/ecpaylogistic.inc.php");
		$sFieldName = 'code';
		...
		include_once("admin/controller/extension/shipping/ECPay.Logistics.Integration.php");
		...
		$al_srvreply = (($_SERVER['HTTPS']) ? "https" : "http") . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '/index.php?route=extension/shipping/ecpaylogistic/set_store_info';
	...
	public function show_cvs_map_by_order() {
		...
		if ($aorder_info) {
			include_once("admin/controller/extension/shipping/ECPay.Logistics.Integration.php");
			include_once("admin/controller/extension/shipping/ecpaylogistic.inc.php");
			$sServerReply = (($_SERVER['HTTPS']) ? "https" : "http") . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '/index.php?route=extension/shipping/ecpaylogistic/set_store_info_by_order';
	...
	public function response() {
		include_once("admin/controller/extension/shipping/ECPay.Logistics.Integration.php");
		include_once("admin/controller/extension/shipping/ecpaylogistic.inc.php");
		include_once("admin/model/extension/payment/EcpayCartInvoiceLibrary.php");
		$sFieldName = 'code';
		if (!ecpay_column_exists($this->db, DB_PREFIX."setting", 'code')) {
			$sFieldName = 'group';
		} 
		...
		try {
			$oInvoiceHelper = new EcpayCartInvoiceLibrary();
			...
			if (in_array($sRtnCode, $aSuccessCodes)) {
				...
				if (in_array($shippingCode[1], $shippingMethod)) {
					// Check invoice module status
                    $sOpayInvoiceStatus = $this->config->get('opayinvoice_status');
                    $sEcpayInvoiceStatus = $this->config->get('ecpayinvoice_status');

					$sInvoiceService = $oInvoiceHelper->getInvoiceService($sOpayInvoiceStatus, $sEcpayInvoiceStatus);
					if ($sInvoiceService !== '') {
                        $sInvoiceModelName = 'model_extension_payment_' . $sInvoiceService . 'invoice';
                        $this->load->model('extension/payment/' . $sInvoiceService . 'invoice');
                        $sInvoiceAutoissue = $this->config->get($sInvoiceService . 'invoice_autoissue');
                        $sSdkPath = $this->$sInvoiceModelName->check_invoice_sdk();
                        if($sInvoiceAutoissue === '1' and $sSdkPath != false) {    
                            $this->$sInvoiceModelName->createInvoiceNo($orderID, $sSdkPath);
                        }
                    }
				}
			}
